...
class SuccessHeaders200_get_status(ModelTaskBaseResponse):
    """
...
class Success200_get_status(ModelCachedSuccess):
    """
    Server status
    ...
    headers: SuccessHeaders200_get_status = Field(
        ..., description='The response headers for this request.'
    )
    players: int = Field(default=..., description="Current online player count")
    server_version: str = Field(default=..., description="Running version as string")
    start_time: str = Field(default=..., description="Server start timestamp")
    vip: bool | None = Field(description="If the server is in VIP mode")


class get_status(TaskESI):
    """
    ...
    def get_sync(
        ...
        :return: The response from ESI as a pydantic object. The response model will follow the structure of :class:`Success200_get_status`.
        """
    ...
    def run(self, datasource: str = "tranquility", **kwargs) -> dict:
        ...
        :return: The response from ESI as a JSON dictionary. The response dictionary will follow the structure of :class:`Success200_get_status`.
        """
